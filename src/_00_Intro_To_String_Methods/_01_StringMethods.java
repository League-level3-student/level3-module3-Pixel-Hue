...
public class _01_StringMethods {
	...
	public static int wordsEndsWithSubstring(String s, String substring) {
		...
		int count = 0;
		// No punctuation marks so can simply add space only
		String checker = substring + " ";

		for (int i = s.length() - substring.length() - 1; 0 <= i; i--) {

//			if (s.substring(i, i + 1).equals(" ")) {
//				checkWord = true;
//			}
//			if (checkWord) {
			if (s.substring(i, i + substring.length() + 1).equals(checker)) {
				count++;
				// checkWord = false;
			}
			// }
...
//last index
	...
	public static boolean palindrome(String s) {
		...
		for (int i =0; i < newS.length(); i++) {
			...
			if (!(newS.substring(i, i+1).equalsIgnoreCase(newS.substring(newS.length()-1-i, newS.length()-i)))) {
				palindrome = false;
			}
		}
